...
try {
    ...
    foreach ($gameId in @('ffviiold', 'ffviinew')) {
        ...
        Assert-Equal '0.1.10' $latest.version "$gameId must publish v0.1.10 as its newest release."
        Assert-Equal 'beta' $latest.channel "$gameId v0.1.10 must be a beta release."
        Assert-True ($latest.packageUrl -match ("/v0\.1\.10/$gameId-v0\.1\.10-amm\.zip$")) "$gameId v0.1.10 points to the wrong package asset."
        Assert-True ($latest.sha256 -match '^[0-9a-f]{64}$') "$gameId v0.1.10 must have a SHA-256 digest."
    }
