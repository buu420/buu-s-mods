...
try {
    ...
    foreach ($gameId in @('ffviiold', 'ffviinew')) {
        ...
        $matches = @($releases | Where-Object version -eq '0.2.8')
        Assert-Equal 1 $matches.Count "$gameId must publish exactly one v0.2.8 release."
        $latest = $matches[0]
        Assert-Equal 'beta' $latest.channel "$gameId v0.2.8 must be a beta release."
        Assert-True ($latest.packageUrl -match ("/v0\.2\.8/$gameId-v0\.2\.8-amm\.zip$")) "$gameId v0.2.8 points to the wrong package asset."
        Assert-True ($latest.sha256 -match '^[0-9a-f]{64}$') "$gameId v0.2.8 must have a SHA-256 digest."
    }
